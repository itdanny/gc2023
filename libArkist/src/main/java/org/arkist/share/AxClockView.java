 ...
 * indicator.
 */
public class AxClockView extends CompoundButton {
    private int mMax;
    ...
    public AxClockView(Context context) {
        super(context);
        init(context, null, 0);
    }

    public AxClockView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs, 0);
    }

    public AxClockView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        int circleColor = res.getColor(R.color.clock_view_default_circle_color);
        int progressColor = res.getColor(R.color.clock_view_default_progress_color);

        if (attrs != null) {
            // Attribute initialization
            final TypedArray a = context
                    .obtainStyledAttributes(attrs, R.styleable.AxClockView,
                            defStyle, 0);

            mMax = a.getInteger(R.styleable.AxClockView_max, mMax);
            mProgress = a.getInteger(R.styleable.AxClockView_progress, mProgress);

            circleColor = a.getColor(R.styleable.AxClockView_circleColor, circleColor);
            progressColor = a.getColor(R.styleable.AxClockView_progressColor, progressColor);

            a.recycle();
        }

        // Other initialization
        mPinnedDrawable = res.getDrawable(R.drawable.clock_view_pinned);
        mPinnedDrawable.setCallback(this);
        mUnpinnedDrawable = res.getDrawable(R.drawable.clock_view_unpinned);
        mUnpinnedDrawable.setCallback(this);
        mShadowDrawable = res.getDrawable(R.drawable.clock_view_shadow);
        mShadowDrawable.setCallback(this);

        mDrawableSize = mShadowDrawable.getIntrinsicWidth();
        mInnerSize = res.getDimensionPixelSize(R.dimen.clock_view_inner_size);
    ...
    /**
     * A {@link Parcelable} representing the {@link AxClockView}'s state.
     */
